...
- 이름: 민준 (가상 사용자)
- 연령: 23세
- 유형: 개인 창작 사용자 (감정 셀프 체크 + 창작 톤 점검)
- 특징: 감정 표현이 서툴지만 그림 그리기를 좋아하고, 색 조합의 정서적 인상을 확인하고 싶어함
- 목표 1: 오늘 자신의 감정 상태를 빠르게 확인하고 기록하고 싶음
- 목표 2: 내가 만든 색 조합이 의도한 분위기와 맞는지 점검하고 싶음
...
### 단계 2. 그림 작성
- 사용자 목표: 현재 기분과 작업물의 정서 톤을 함께 표현하기
- 사용자 행동: 캔버스에서 2~4개 대표 색상을 선택하고 드로잉
- 시스템 반응: 선택 색상 및 대표 팔레트 미리보기를 실시간 제공
- 사용자 감정: 몰입, 창의성
...
### 단계 4. 결과 확인
- 사용자 목표: 예측 감정이 내 의도한 톤과 일치하는지 확인하기
- 사용자 행동: 예측 감정 배지, 상위 감정 점수, 대표 색상 스와치 확인
...
### 단계 6. 기록 확인 및 재방문
- 사용자 목표: 과거 감정 흐름과 창작 톤 패턴 확인하기
- 사용자 행동: 히스토리 화면에서 날짜별 결과와 예측/수정 감정 비교 조회
...
시나리오명: 오늘 감정과 창작 톤을 함께 점검하는 흐름

1. 사용자는 앱 실행 후 홈 화면에서 새 그림 시작을 선택한다.
2. 최근 분석 요약을 확인해 현재 맥락을 파악한 뒤 캔버스로 진입한다.
3. 캔버스에서 주황/하늘색 중심의 팔레트로 감정과 의도한 분위기를 표현한다.
4. 하단 팔레트 미리보기를 보며 대표 색 조합이 의도와 맞는지 1차 확인한다.
5. 분석하기를 누르고 단계형 로딩 메시지(색상 추출 중 -> 점수 계산 중)를 확인한다.
6. 2초 내 분석 결과 화면에서 예측 감정(예: energy)과 상위 감정 점수 분포를 확인한다.
7. 예측이 의도와 일치하면 결과를 저장하고 기록에 반영한다.
8. 예측이 다르면 실제 감정을 선택하고 짧은 메모와 함께 피드백을 제출한 뒤 기록을 확인한다.
